...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const reasoningEfforts = useMemo(
    () => normalizeReasoningEfforts(mainCaps?.reasoning_efforts),
  ...
  const effortValue =
    rawEffort === 'false' || rawEffort === 'disabled'
      ? 'none'
      : normalizeReasoningEffort(rawEffort || 'medium', reasoningEfforts)
            ...
            {reasoningSupported && (
              <div className="flex items-center gap-2 text-xs">
                ...
                >
                  ...
                  <SelectContent>
                    {['none', ...reasoningEfforts].map(value => (
                      <SelectItem key={value} value={value}>
                        {value === 'none' ? m.reasoningOff : reasoningEffortLabel(value)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
